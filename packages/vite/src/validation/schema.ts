...
/**
 * Required fields per preset type.
 */
...
/**
 * Recommended (but not required) fields per preset type.
 */
...
/**
 * Validate a schema config for completeness.
 */
export function validateSchema(
  ...
  if (!('preset' in schema) || !schema.preset) {
    // Custom schema — just check @type exists
    if (!schema['@type']) {
      results.push({
        severity: 'error',
