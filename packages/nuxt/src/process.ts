// NOTE: This flat-directory processor is intentionally a verbatim copy of
// `@agentmarkup/cli`'s `src/process.ts`. Both adapters process a single static
// output directory identically; the planned convergence is to lift this into a
// Node-only `@agentmarkup/core` entry point (kept out of core's main entry so
// the browser-safe surface the website imports stays free of `node:fs`). Until
// that extraction, keep this file in sync with the CLI copy.
